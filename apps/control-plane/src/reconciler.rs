...
use spaas_protocol::job::JobState;
...
pub async fn run_reconciler_loop(state: AppState) {
    ...
    loop {
        ...
        // 2. Identify jobs assigned to dead nodes and recover/retry
        let mut jobs_to_reschedule = Vec::new();
        {
            ...
            for (job_id, job) in jobs.iter_mut() {
                if job.state == JobState::Scheduled || job.state == JobState::Running {
                    let should_fail_or_retry = if let Some(assigned_id) = job.assigned_node_id {
                    ...
                    if should_fail_or_retry || is_timed_out {
                        if job.retry_count < job.spec.retry_policy.max_retries {
                            warn!(
                                job_id = %job_id,
                                retry = job.retry_count + 1,
                                max = job.spec.retry_policy.max_retries,
                                "Node disappeared or job timed out; recovering and rescheduling job"
                            );
                            ...
                            warn!(
                                job_id = %job_id,
                                "Job exceeded max retries after node disconnection; marking FAILED"
                            );
                            let _ = job.transition_to(JobState::Failed);
                            job.error_message = Some("Node disappeared and retry threshold reached".into());
            ...
            if let Some(job) = jobs.get_mut(&q_id) {
                if let Ok(selected) = state.scheduler.schedule_workload(&job.spec, &eligible_nodes) {
                    if !selected.is_empty() {
                        ...
                        info!(job_id = %q_id, node_id = %primary, "Queued job scheduled by reconciler");
                    }
                }
            }
        }
